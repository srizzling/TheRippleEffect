...
class VoteManager(models.Manager):
    ...
    def get_weighted_score(self, obj, user=None):
        if not obj:
            return []
        score_raw = self.get_score(obj)
        i = obj._get_pk_val()

        if not (hasattr(obj, 'weight') and callable(getattr(obj, 'weight'))):
            raise AttributeError('get_weighted_score requires the '
                                 'object being voted on define a "weight" '
                                 'method, taking as a parameter the sum of '
                                 'votes and number of votes.')
        score = {
            "object": obj,
            "id": i,
            "score": score_raw["score"],
            "num_votes": score_raw["num_votes"],
            "weight": obj.weight(score_raw["score"], score_raw["num_votes"]),
        }

        if user:
            user_dict = self.get_for_user(obj, user)
            score["user_vote"] = user_dict[score["id"]] if score["id"] in user_dict else 0
        return score
    ...
    def get_weighted_scores_in_bulk(self, objects, user=None):
        objects_ids = [(o, o._get_pk_val()) for o in objects]
        if not objects_ids:
            return []
        scores_dict = self.get_scores_in_bulk(objects)

        obj = objects[0]
        if not (hasattr(obj, 'weight') and callable(getattr(obj, 'weight'))):
            raise AttributeError('get_weighted_scores_in_bulk requires the '
                                 'object being voted on define a "weight" '
                                 'method, taking as a parameter the sum of '
                                 'votes and number of votes.')
        scores = [{
                "object": o,
                "id": i,
                "score": scores_dict[i]["score"],
                "num_votes": scores_dict[i]["num_votes"],
                "weight": o.weight(scores_dict[i]["score"], scores_dict[i]["num_votes"]),
            } if i in scores_dict else {
                "object": o,
                "id": i,
                "score": 0,
                "num_votes": 0,
                "weight": o.weight(0, 0),
            } for o, i in objects_ids]

        scores.sort(key=lambda e: e["weight"], reverse=True)
        if user:
            user_dict = self.get_for_user_in_bulk(objects, user)
            for i in scores:
                i["user_vote"] = user_dict[i["id"]] if i["id"] in user_dict else 0
        return scores
